#!/bin/bash
# vendor/guardtalk/scripts/stage-laguna-release.sh
#
# T-PORT-LAGUNA-HYBRID (2026-09-23) — device-generic laguna release stager.
# Generalises the proven rango recipe in stage-rango-release.sh (which stays
# archived/unmodified) to any laguna device:
#
#   rango (Pixel 10 Pro Fold), frankel (Pixel 10),
#   blazer (Pixel 10 Pro), mustang (Pixel 10 Pro XL).
#
# WHY: `blazer`'s shipped stamp is a plain `fullgt` composition. The rango
# stager records that a fully-GuardTalkOS laguna boot chain (`fullgt`) was
# ABL-rejected on-device 2026-08-02, and that `com.android.virt.apex` is an
# EARLY_VM bootstrap landmine (0xfc). The only laguna artifact that reaches
# apexd is the *hybrid*: factory boot chain + factory system_dlkm/vendor_dlkm
# + GuardTalkOS system/system_ext/product/vendor + virt-drop + stock
# bootstrap-apex grafts (runtime/i18n/tzdata). This script ports that recipe
# with every rango path device-parameterised. Nothing defaults to rango.
#
# Usage:
#   stage-laguna-release.sh <rango|frankel|blazer|mustang> [--out DIR] [--link-latest]
#
#   <device>       REQUIRED. Selects OUT/release/donor paths. There is no
#                  implicit rango default.
#   --out DIR      Write the bundle to DIR instead of
#                  releases/desktop-flash/<device>-YYYYmmdd-HHMMSS.
#   --link-latest  Point releases/desktop-flash/<device>-latest at this stamp.
#                  Refused for every bisect/diagnostic MODE (see dispatch).
#                  Cannot be combined with --out.
#
# Donors read (provisioned by the operator, see work item 2 of the dispatch):
#   releases/desktop-flash/<device>-stock-userspace/   factory userspace/img set
#   releases/desktop-flash/<device>-stock-control/     factory bootloader/radio
#   releases/desktop-flash/<device>-rescue-boot/       factory boot chain
# Their provenance is recorded in each donor's README.md.
#
...
#              May repoint $DEV-latest with --link-latest.
...
#              donor (releases/desktop-flash/$DEV-stock-userspace,
...
#              (releases/desktop-flash/$DEV-stock-userspace, factory
...
#              --link-latest refused (must not become $DEV-latest again).
#
#   fullgt     Fully coherent OUT including GT boot chain (H3 ABL test).
#              NEVER linked to $DEV-latest by this script.
#
...
#
# Usage:
#   ./stage-laguna-release.sh <dev> --link-latest          # MODE=gtuserspace (default)
#   MODE=stockbootstrap ./stage-laguna-release.sh <dev>   # keep virt (103641-class)
#   MODE=stockbootapex ./stage-laguna-release.sh <dev>    # legacy: keep virt + stock apex
#   MODE=novirtstockapex ./stage-laguna-release.sh <dev>  # drop virt + stock runtime/i18n
#   MODE=stockapexd ./stage-laguna-release.sh <dev>       # durable gtuserspace + stock apexd
#   MODE=stockinit ./stage-laguna-release.sh <dev>        # stockapexd + stock /system/bin/init (FALSIFIED)
#   MODE=stockapexcfg ./stage-laguna-release.sh <dev>     # stockapexd + GT init + stock init.rc (FALSIFIED)
#   MODE=stockhwasan ./stage-laguna-release.sh <dev>      # stockapexcfg + stock bootstrap/hwasan libc (FALSIFIED)
#   MODE=stockselinux ./stage-laguna-release.sh <dev>     # stockhwasan + stock early-apex SELinux (FALSIFIED)
#   MODE=stockapexset ./stage-laguna-release.sh <dev>     # durable gtuserspace + stock /system/apex wholesale (FALSIFIED)
#   MODE=stockvendor ./stage-laguna-release.sh <dev>      # durable gtuserspace + factory vendor.img wholesale
#   MODE=gtsystemonstockinitrc ./stage-laguna-release.sh <dev>  # inverted + stock init.rc (FALSIFIED)
#   MODE=gtsystemonstockplatctx ./stage-laguna-release.sh <dev> # inverted + leftover plat contexts (FALSIFIED)
#   MODE=gtsystemonstockearlyinit ./stage-laguna-release.sh <dev> # inverted + leftover early-init rc/bins (FALSIFIED)
#   MODE=gtsystemonstockprotobuf ./stage-laguna-release.sh <dev> # inverted + stock protobuf (class change 0x100)
#   MODE=gtsystemonstocklibcxx ./stage-laguna-release.sh <dev> # inverted + stock libc++ (0xfc; not a cure)
#   MODE=gtsystemonstockldandroid ./stage-laguna-release.sh <dev> # inverted + stock ld-android (0xfc)
#   MODE=gtsystemonstockapexdetc ./stage-laguna-release.sh <dev> # inverted + stock /system/etc/apexd (0xfc)
#   MODE=gtsystemonstocktaskprof ./stage-laguna-release.sh <dev> # inverted + stock task_profiles.json (0xfc)
#   MODE=gtsystemonstockaconfig ./stage-laguna-release.sh <dev> # inverted + stock /system/etc/aconfig (0xfc)
#   MODE=gtsystemonstockacflags ./stage-laguna-release.sh <dev> # inverted + stock aconfig_flags.pb (0xfc)
#   MODE=gtsystemonstockbflags ./stage-laguna-release.sh <dev> # inverted + stock build_flags.json (061630 0xfc; series CLOSED)
#   MODE=hybrid ./stage-laguna-release.sh <dev>            # legacy graft diagnostic
#   MODE=fullgt ./stage-laguna-release.sh <dev>
#   MODE=avbcontrol ./stage-laguna-release.sh <dev>
#
# Exit code: 0 on success. Non-zero + no $DEV-latest change on any gate
...
# ---------------------------------------------------------------------------
# Device argument (REQUIRED — no implicit rango default) + options.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-LAGUNA-KLOG-DUMP (diagnostic): opt-in, default OFF.
# When GT_KLOD_DUMP=1 the durable MODE=gtuserspace system.img additionally gets
# /system/etc/gt-klogdump.sh + /system/etc/init/gt-klogdump.rc and an async
# `start gt-klogdump` immediately before `exec_start apexd-bootstrap`, so a
# 0xfc reboot to the bootloader still leaves the kernel log in `metadata`,
# readable from the host via `fastboot fetch metadata`. When OFF the
# gtuserspace composition is unchanged (nothing new is written).
# Only valid for MODE=gtuserspace; never linked to $DEV-latest (diagnostic).
# ---------------------------------------------------------------------------
GT_KLOD_DUMP="${GT_KLOD_DUMP:-0}"
case "$GT_KLOD_DUMP" in
  0|1) ;;
  *) echo "[stage] GT_KLOD_DUMP must be 0 or 1 (got '$GT_KLOD_DUMP')" >&2; exit 2 ;;
esac
if [ "$GT_KLOD_DUMP" = "1" ]; then
  [ "$MODE" = "gtuserspace" ] \
    || { echo "[stage] GT_KLOD_DUMP=1 is supported only for MODE=gtuserspace (got MODE=$MODE)" >&2; exit 2; }
  [ "$LINK_LATEST" = "1" ] \
    && { echo "[stage] GT_KLOD_DUMP=1 is a diagnostic build — refuse --link-latest" >&2; exit 2; }
fi

OUT="$ROOT/out/target/product/$DEV"
STOCK="$ROOT/releases/desktop-flash/$DEV-stock-userspace"
STOCKCTL="$ROOT/releases/desktop-flash/$DEV-stock-control"
RESCUE="$ROOT/releases/desktop-flash/$DEV-rescue-boot"
HOST_BIN="$ROOT/out/host/linux-x86/bin"
WORK="${STAGE_WORK:-/tmp/$DEV-stage-$MODE}"
if [ -z "${STAMP:-}" ]; then
  STAMP="$DEV-$(date -u +%Y%m%d-%H%M%S)"
  # Diagnostic klog bundles get a distinct, self-describing stamp (never -latest).
  [ "$GT_KLOD_DUMP" = "1" ] && STAMP="$STAMP-klog"
fi
...
# Donor build id for README provenance. Written by the provisioning step as
# <donor>/STOCK_ID; env STOCK_ID wins. Never assumed to be rango's CP1A.
if [ -z "${STOCK_ID:-}" ] && [ -f "$STOCK/STOCK_ID" ]; then
  STOCK_ID="$(head -1 "$STOCK/STOCK_ID" | tr -d '\r\n')"
fi
STOCK_ID="${STOCK_ID:-unknown}"
# AOSP testkey (== avb_pkmd.bin) used to derive a flags:3 vbmeta when a device
# has no prebuilt flags3 blob yet. Same key as the archived rango artifact.
AVB_TESTKEY="${AVB_TESTKEY:-$ROOT/external/avb/test/data/testkey_rsa4096.pem}"
# laguna platform super geometry (vendor/adevtool/config/mk/google_devices/
# common/BoardConfig-common.mk: 8531214336 = SUPER_PARTITION_SIZE,
# 8527020032 = SUPER_PARTITION_SIZE - 4MiB overhead). Platform-wide, not
# device-specific; env-overridable for a future geometry change.
LAGUNA_SUPER_SIZE="${LAGUNA_SUPER_SIZE:-8531214336}"
LAGUNA_GROUP_SIZE="${LAGUNA_GROUP_SIZE:-8527020032}"

log() { echo "[stage:$DEV:$MODE] $*"; }
die() { echo "[stage:$DEV:$MODE] ERROR: $*" >&2; exit 1; }
...
# ---------------------------------------------------------------------------
# AVB inputs (T-PORT-LAGUNA-HYBRID).
# The archived rango recipe shipped a hand-built testkey vbmeta with
# disable-verity + disable-verification baked in (flags=3) plus the matching
# avb_pkmd. Both are resolved from THIS device's own tree; nothing falls back
# to another device's stamp. rango keeps its existing prebuilt flags3 blob
# (tried first). blazer/frankel/mustang have no flags3 blob yet, so one is
# derived from the device's own testkey-signed vbmeta.img (same descriptors,
# same AOSP testkey as avb_pkmd.bin, --flags 3). The flash step also passes
# --disable-verity --disable-verification, so this is belt-and-suspenders.
# ---------------------------------------------------------------------------
VBMETA_FLAGS3=""
PKMD=""
VBMETA_TMPDIR=""
resolve_laguna_avb_inputs() {
  local c src d outdir
  VBMETA_FLAGS3=""
  for c in \
    "$ROOT/releases/desktop-flash/$DEV-latest/vbmeta_valid_flags3.img" \
    "$STOCK/vbmeta_valid_flags3.img" \
    "$STOCKCTL/vbmeta_valid_flags3.img" \
    "$ROOT/releases/desktop-flash/$DEV-memtagfix2-20260728-101648/vbmeta_valid_flags3.img" \
    "$ROOT/releases/desktop-flash/$DEV-avbcontrol-20260801-072014/vbmeta_valid_flags3.img"
  do
    if [ -f "$c" ]; then VBMETA_FLAGS3="$c"; break; fi
  done

  if [ -z "$VBMETA_FLAGS3" ]; then
    src=""
    for d in \
      "$ROOT/releases/desktop-flash/$DEV-latest/vbmeta.img" \
      "$STOCK/vbmeta.img" \
      "$STOCKCTL/vbmeta.img" \
      "$RESCUE/vbmeta.img"
    do
      if [ -f "$d" ]; then src="$d"; break; fi
    done
    [ -n "$src" ] || die "no vbmeta_valid_flags3.img and no source vbmeta.img for $DEV (provision $DEV-stock-userspace first)"
    [ -x "$HOST_BIN/avbtool" ] || die "avbtool missing at $HOST_BIN/avbtool — required to derive a flags:3 vbmeta for $DEV"
    need "$AVB_TESTKEY"
    outdir="$(mktemp -d "/tmp/$DEV-vbmeta.XXXXXX")"
    VBMETA_TMPDIR="$outdir"
    VBMETA_FLAGS3="$outdir/vbmeta_valid_flags3.img"
    log "  no prebuilt flags3 vbmeta for $DEV — deriving from $src (--flags 3, AOSP testkey)"
    # --padding_size MUST equal the laguna vbmeta partition size (16384 B).
    # Verified against every stock laguna factory image: rango/blazer/frankel/
    # mustang all ship vbmeta.img = 16384 B, and the one proven-booting hybrid
    # (rango-20260802-130756) ships 16384 B for all three vbmeta images.
    # Without this avbtool emits ~7616 B, leaving 8768 B of stale partition
    # content from a previous (possibly fullgt, flags:0) flash.
    # Ref: T-PORT-LAGUNA-HYBRID Addendum 4 / B-PORT-LAGUNA-HYBRID-VERIFY.
    "$HOST_BIN/avbtool" make_vbmeta_image \
      --include_descriptors_from_image "$src" \
      --flags 3 --key "$AVB_TESTKEY" --algorithm SHA256_RSA4096 \
      --padding_size 16384 \
      --output "$VBMETA_FLAGS3" \
      || die "avbtool make_vbmeta_image failed for $src"
    log "  derived flags3 vbmeta: $VBMETA_FLAGS3"
  else
    log "  flags3 vbmeta source: $VBMETA_FLAGS3"
  fi

  PKMD=""
  for c in \
    "$ROOT/releases/desktop-flash/$DEV-latest/avb_pkmd.bin" \
    "$ROOT/releases/desktop-flash/$DEV-latest/avb_pkmd_testkey.bin" \
    "$STOCK/avb_pkmd_testkey.bin" \
    "$STOCK/avb_pkmd.bin" \
    "$STOCKCTL/avb_pkmd.bin" \
    "$RESCUE/avb_pkmd.bin" \
    "$ROOT/releases/desktop-flash/$DEV-memtagfix2-20260728-101648/avb_pkmd_testkey.bin" \
    "$ROOT/releases/desktop-flash/$DEV-avbcontrol-20260801-072014/avb_pkmd.bin"
  do
    if [ -f "$c" ]; then PKMD="$c"; break; fi
  done
  [ -n "$PKMD" ] || die "avb_pkmd testkey not found for $DEV (provision the donor dirs or a $DEV-latest stamp)"
  log "  avb_pkmd source: $PKMD"
}
...
mte_strip_gate() {
  ...
  local bc="$ROOT/vendor/guardtalk/device/$DEV/BoardConfig-excised-late.mk"
  need "$bc"
  ...
  local memtag_mk="$ROOT/vendor/guardtalk/device/$DEV/guardtalk-memtag.mk"
  need "$memtag_mk"
...
patch_system_img_for_factory_boot() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-syspatch.XXXXXX)"
...
patch_stock_bootstrap() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-stockboot.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_bootstrap_apexes() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-stockapex.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
# T-LAGUNA-KLOG-DUMP (diagnostic; GT_KLOD_DUMP=1 only) — inject a background
# dmesg dumper into system.img so a boot that dies at apexd-bootstrap (0xfc,
# reboot bootloader) still leaves the failing kernel log in the `metadata`
# partition, recoverable from the host with `fastboot fetch metadata`.
#
# Why this shape (dispatch T-LAGUNA-KLOG-DUMP, DEC-PORT-GEN8910-WAVE4 §D3/D4):
#   * `dmesg` (toybox: opens /dev/kmsg O_NONBLOCK, reads without clearing the
#     ring) — non-destructive and non-blocking. NEVER `cat /proc/kmsg`.
#   * raw block device via a `>` redirect; the dumper mounts nothing.
#   * service started with async `start`, NOT `exec_start`; NOT `oneshot`.
#   * `reboot_on_failure` stays on apexd-bootstrap — the reboot is what returns
#     the device to the bootloader so `fastboot fetch` can run.
#   * same init.rc anchor as the restorecon patch: immediately before
#     `# Run apexd-bootstrap`.
# Writes use the proven debugfs rm+write+set_inode_field pattern and are
# cmp-verified per file; the grafted stock bootstrap apexes are re-verified
# byte-for-byte after the writes (debugfs 1.47 write-order bug class).
patch_klog_dump() {
  local img="$1"
  need "$img"
  command -v debugfs >/dev/null || die "debugfs required for gt-klogdump patch"

  local tmp
  tmp="$(mktemp -d /tmp/$DEV-klogdump.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN

  log "Patching system.img: gt-klogdump diagnostic (T-LAGUNA-KLOG-DUMP)..."

  # 1) Dumper script: `dmesg` only; raw block device; loop; end marker.
  cat > "$tmp/gt-klogdump.sh" <<'KLODSH'
#!/system/bin/sh
# GuardTalk laguna diagnostic (T-LAGUNA-KLOG-DUMP) — DIAGNOSTIC, DO NOT SHIP.
#
# Persist the kernel log to the RAW `metadata` block device. The GT boot dies
# at apexd-bootstrap and reboots to the bootloader (~17s, 0xfc); this loop
# starts first so the final iteration holds the failure, recoverable on the
# host with `fastboot fetch metadata`.
#
# `dmesg` only: toybox dmesg opens /dev/kmsg O_NONBLOCK and reads without
# clearing the ring. NEVER `cat /proc/kmsg` (destructive + blocking).
# Nothing is mounted here: `>` writes raw bytes from offset 0.

# Resolve the raw metadata device. metadata is first_stage_mount, so
# /dev/block/by-name/metadata is normally present before early-init; fall back
# to the platform by-name path if ueventd has not published the symlink yet.
META=""
n=0
while [ $n -lt 10 ]; do
  for c in \
      /dev/block/by-name/metadata \
      /dev/block/platform/*/by-name/metadata \
      /dev/block/platform/*/*/by-name/metadata; do
    if [ -b "$c" ]; then META="$c"; break; fi
  done
  [ -n "$META" ] && break
  n=$((n + 1))
  sleep 1
done
[ -n "$META" ] || META=/dev/block/by-name/metadata

i=0
while [ $i -lt 120 ]; do
  {
    echo "=== GT-KLOG iter=$i ts=$(date +%s) dev=$META ==="
    dmesg
    echo "=== GT-KLOG-END iter=$i ==="
  } > "$META" 2>/dev/null
  i=$((i + 1))
  sleep 1
done
KLODSH
  ...
  # 2) Service definition. `class core`; NOT oneshot; seclabel su:s0 — `su` is
  #    the only permissive domain in the shipped vendor precompiled_sepolicy
  #    (verified: `sepolicy-analyze <policy> permissive` -> `su`), so the raw
  #    block-device write is permitted.
  cat > "$tmp/gt-klogdump.rc" <<'KLODRC'
# GuardTalk laguna diagnostic (T-LAGUNA-KLOG-DUMP) — DIAGNOSTIC, DO NOT SHIP.
# Background dmesg dumper; started asynchronously before apexd-bootstrap.
# NOT oneshot: the loop must outlive the failure. seclabel su:s0 is the
# permissive domain in this build's policy (block-device write allowed).
service gt-klogdump /system/bin/sh /system/etc/gt-klogdump.sh
    class core
    user root
    group root system
    seclabel u:r:su:s0
KLODRC
  ...
  # 3) init.rc: insert async `start` immediately before `# Run apexd-bootstrap`
  #    (same insertion point as the restorecon patch; the anchor is stable
  #    whether or not restorecon was inserted first).
  debugfs -R "dump /system/etc/init/hw/init.rc $tmp/init.rc" "$img" >/dev/null 2>&1 \
    || die "gt-klogdump: dump init.rc failed"
  grep -q 'start gt-klogdump' "$tmp/init.rc" \
    && die "gt-klogdump: init.rc already has start gt-klogdump (unexpected)"
  python3 - "$tmp/init.rc" <<'PY' || die "gt-klogdump: init.rc insert failed"
import pathlib, sys
p = pathlib.Path(sys.argv[1])
text = p.read_text()
needle = "\n    # Run apexd-bootstrap"
insert = (
    "\n    # GuardTalk laguna klog diagnostic (T-LAGUNA-KLOG-DUMP): async\n"
    "    # dmesg loop; survives a 0xfc reboot to the bootloader.\n"
    "    start gt-klogdump\n\n"
    "    # Run apexd-bootstrap"
)
if text.count(needle) != 1:
    raise SystemExit("init.rc anchor '# Run apexd-bootstrap' not found exactly "
                     "once (got %d)" % text.count(needle))
p.write_text(text.replace(needle, insert, 1))
PY

  # Capture the grafted stock bootstrap apexes before the writes so the debugfs
  # 1.47 write-order corruption class can be excluded afterwards.
  local a pre_apexes=""
  for a in com.android.tzdata.apex com.android.runtime.apex com.android.i18n.apex; do
    if debugfs -R "stat /system/apex/$a" "$img" >/dev/null 2>&1; then
      debugfs -R "dump /system/apex/$a $tmp/pre-$a" "$img" >/dev/null 2>&1 \
        || die "gt-klogdump: pre-dump $a failed"
      pre_apexes="$pre_apexes $a"
    fi
  done

  # 4) Write the three files (proven pattern; cmp-verified per file).
  _debugfs_graft_file "$img" "/system/etc/gt-klogdump.sh" "$tmp/gt-klogdump.sh" 0x81ed 0 0
  _debugfs_graft_file "$img" "/system/etc/init/gt-klogdump.rc" "$tmp/gt-klogdump.rc" 0x81a4 0 0
  _debugfs_graft_file "$img" "/system/etc/init/hw/init.rc" "$tmp/init.rc" 0x81a4 0 0
  log "  wrote /system/etc/gt-klogdump.sh (0755) + /system/etc/init/gt-klogdump.rc (0644) ✓"

  # 5) Post-write integrity: grafted apexes byte-identical; virt absent.
  for a in $pre_apexes; do
    debugfs -R "dump /system/apex/$a $tmp/post-$a" "$img" >/dev/null 2>&1 \
      || die "gt-klogdump: post-dump $a failed"
    cmp -s "$tmp/pre-$a" "$tmp/post-$a" \
      || die "gt-klogdump: $a corrupted by debugfs writes (cmp mismatch)"
  done
  if ! debugfs -R "stat /system/apex/com.android.virt.apex" "$img" 2>&1 | grep -q 'File not found'; then
    die "gt-klogdump: com.android.virt.apex unexpectedly present after injection"
  fi
  log "  bootstrap apexes re-verified byte-identical; virt.apex absent ✓"

  # 6) Verify the injected start sits immediately before apexd-bootstrap.
  debugfs -R "dump /system/etc/init/hw/init.rc $tmp/verify-init.rc" "$img" >/dev/null 2>&1 \
    || die "gt-klogdump: verify dump init.rc failed"
  grep -q 'start gt-klogdump' "$tmp/verify-init.rc" \
    || die "gt-klogdump: start gt-klogdump missing after write"
  grep -A2 'start gt-klogdump' "$tmp/verify-init.rc" | grep -q '# Run apexd-bootstrap' \
    || die "gt-klogdump: start gt-klogdump not immediately before # Run apexd-bootstrap"
  grep -q 'exec_start apexd-bootstrap' "$tmp/verify-init.rc" \
    || die "gt-klogdump: exec_start apexd-bootstrap missing after write"
  log "  init.rc: start gt-klogdump immediately before # Run apexd-bootstrap; exec_start apexd-bootstrap intact ✓"
}
...
patch_stock_apexd() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-stockapexd.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_ueventd_rc() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-ueventd.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_build_prop() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-buildprop.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_apexd_needed_libs() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-apexdlibs.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_plat_property_contexts() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-pctx.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_remaining_plat_contexts() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-platctx.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_early_init_surface() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-earlyinit.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_protobuf() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-protobuf.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_libcxx() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-libcxx.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_ldandroid() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-ldandroid.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_apexdetc() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-apexdetc.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_taskprof() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-taskprof.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_aconfig() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-aconfig.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_acflags() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-acflags.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_bflags() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-bflags.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_init() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-stockinit.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_init_needed_libs() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-initlibs.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_init_rc() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-stockapexcfg.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
patch_stock_hwasan_native_deps() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-stockhwasan.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
_debugfs_graft_file() {
  ...
  dir="$(dirname "$path")"
  base="$(basename "$path")"
  dfs="$(mktemp /tmp/$DEV-dfs.XXXXXX)"
  verify="$(mktemp /tmp/$DEV-verify.XXXXXX)"
  {
    echo "cd $dir"
    echo "rm $base"
    echo "write $src $base"
...
patch_stock_selinux_early_apex() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-stockselinux.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
# releases/desktop-flash/$DEV-stock-userspace, factory CP1A.260505.005 —
...
patch_stock_apexset_wholesale() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-stockapexset.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
  ...
  log "  donor: $STOCK/system.img (factory $STOCK_ID) — $n_stock stock apexes dumped ✓"
  ...
  while IFS= read -r apex; do
    ...
    if [ "$sha_src" != "$sha_dst" ]; then
      echo "[stage:$DEV:$MODE] ERROR: sha256 MISMATCH after write: /system/apex/$apex (src $sha_src != img $sha_dst)" >&2
...
# $DEV-stock-userspace, factory CP1A.260505.005 — same donor as APEXSET).
#
...
patch_stock_vendor_wholesale() {
  ...
  local tmp
  tmp="$(mktemp -d /tmp/$DEV-stockvendor.XXXXXX)"
  # shellcheck disable=SC2064
  trap "rm -rf '$tmp'" RETURN
...
link_latest_if_requested() {
  if [ "$LINK_LATEST" = "1" ]; then
    ln -sfn "$STAMP" "$ROOT/releases/desktop-flash/$DEV-latest"
    log "$DEV-latest -> $STAMP"
  else
    log "NOT relinking $DEV-latest (pass --link-latest to do so). Stamp staged at: $DEST"
  fi
}
...
stage_gtuserspace() {
  ...
  # T-LAGUNA-KLOG-DUMP: inject the diagnostic dmesg dumper into the (already
  # grafted) system.img. Runs before lpmake/super and before the bundle copy,
  # so the patched system.img is what ships (and what lands in super.img).
  if [ "$GT_KLOD_DUMP" = "1" ]; then
    patch_klog_dump "$WORK/system.img"
  fi
  ...
  local super_size="$LAGUNA_SUPER_SIZE" group_size="$LAGUNA_GROUP_SIZE"
  ...
  # Clean up a derived flags3 vbmeta temp dir (no-op for prebuilt sources).
  if [ -n "${VBMETA_TMPDIR:-}" ]; then
    rm -rf "$VBMETA_TMPDIR"
    VBMETA_TMPDIR=""
  fi

  # Prefer factory init.insmod with factory dlkm.
  if [ -f "$STOCKCTL/init.insmod.$DEV.cfg" ]; then
    cp -f "$STOCKCTL/init.insmod.$DEV.cfg" "$DEST/init.insmod.$DEV.cfg"
  elif [ -f "$STOCK/init.insmod.$DEV.cfg" ]; then
    cp -f "$STOCK/init.insmod.$DEV.cfg" "$DEST/init.insmod.$DEV.cfg"
  elif [ -f "$ROOT/vendor/guardtalk/device/$DEV/init.insmod.$DEV.cfg" ]; then
    cp -f "$ROOT/vendor/guardtalk/device/$DEV/init.insmod.$DEV.cfg" "$DEST/init.insmod.$DEV.cfg"
  fi
  ...
  case "$MODE" in
    ...
    stockvendor)
      ...
      vendor_blurb="STOCK factory vendor.img WHOLESALE (byte-identical, sha256-gated vs $DEV-stock-userspace donor; e2fsck rc<=2 on throwaway copy; RQ4 #1 loop/ueventd coldboot environment probe — T-RANGO-BOOT-STOCKVENDOR)"
      vendor_table_blurb="Factory CP1A (\`$DEV-stock-userspace\`, byte-identical, sha256-gated)"
      composition_blurb="Factory CP1A **boot + dlkm + vendor** (vendor byte-identical to \`$DEV-stock-userspace\`) + GuardTalkOS **system/system_ext/product** (same OUT)."
      ;;
    stockbootstrap)
  ...
  # Bisect modes must flash via REMOTE_BUILD_DIR (never promote as $DEV-latest).
...
(do **not** promote as \`$DEV-latest\` / do **not** promote stockinit).
...
DEVICE=$DEV ./flash-from-remote.sh
# On fail IMMEDIATELY (before retries drain):
fastboot oem dmesg | tee ~/$DEV-dmesg-apex.txt
...
DEVICE=$DEV ./flash-from-remote.sh
# On fail IMMEDIATELY (before retries drain):
fastboot oem dmesg | tee ~/$DEV-dmesg-deep.txt
# Classify: KP 0x00007f00 vs KP 0x00000100 vs Reboot mode 0xfc
\`\`\`
...
EOF
  ...
  # T-LAGUNA-KLOG-DUMP: make the diagnostic nature and the recovery command
  # unmissable at the very top of the bundle README.
  if [ "$GT_KLOD_DUMP" = "1" ]; then
    local klog_readme="$DEST/README-FLASH-DESKTOP.md"
    {
      cat <<'KLODREADME'
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# !!  DIAGNOSTIC BUILD — EXPECTED TO FAIL TO BOOT — DO NOT PROMOTE         !!
# !!  T-LAGUNA-KLOG-DUMP: captures the failing apexd-bootstrap kernel log  !!
# !!  into the `metadata` partition. Recover with `fastboot fetch`.       !!
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

This is a **diagnostic** build: the durable `MODE=gtuserspace` composition plus
`GT_KLOD_DUMP=1`. It is **expected to fail to boot** at `apexd-bootstrap` and
reboot to the bootloader (~17 s, status `0xfc`). **That reboot is required** —
it is what returns the device to the bootloader so the host can recover the log.
Do **not** treat this as a release; do **not** point `blazer-latest` at it.

Mechanism: init asynchronously starts `gt-klogdump`
(`/system/etc/init/gt-klogdump.rc` -> `/system/etc/gt-klogdump.sh`) immediately
before `exec_start apexd-bootstrap`. The loop runs `dmesg` (never
`cat /proc/kmsg`) once a second to the RAW `metadata` block device
(`/dev/block/by-name/metadata` via a `>` redirect; nothing is mounted).

## Recover the log (host, after the expected failure + reboot to bootloader)

```bash
fastboot fetch metadata gt-klog.img
```
...
The most recent capture is delimited by the LAST marker pair:

```
=== GT-KLOG iter=<N> ts=<epoch> dev=/dev/block/by-name/metadata ===
... verbatim dmesg ...
=== GT-KLOG-END iter=<N> ===
```

`iter=`/`ts=` prove a real capture; bytes after the last `GT-KLOG-END` marker are
stale and must be ignored. The `apexd-bootstrap` failure is in the final
iteration written before the reboot.
...
stage_hybrid() {
  ...
  if [ -f "$OUT/vendor_dlkm/etc/init.insmod.$DEV.cfg" ]; then
    cp -f "$OUT/vendor_dlkm/etc/init.insmod.$DEV.cfg" "$DEST/init.insmod.$DEV.cfg"
  elif [ -f "$ROOT/vendor/guardtalk/device/$DEV/init.insmod.$DEV.cfg" ]; then
    cp -f "$ROOT/vendor/guardtalk/device/$DEV/init.insmod.$DEV.cfg" "$DEST/init.insmod.$DEV.cfg"
  fi

  cat > "$DEST/README-FLASH-DESKTOP.md" <<EOF
# ${DEV} hybrid release — $STAMP

Factory CP1A boot + GuardTalkOS userspace + GT sepolicy graft (RCA §5 proven
interim direction). Produced by \`vendor/guardtalk/scripts/stage-laguna-release.sh\`
(MODE=hybrid) — supersedes the archived one-off \`stage-rango-hybrid.sh\`.

| Partition | Source |
|-----------|--------|
| bootloader / radio | Factory CP1A (\`$DEV-stock-control\`) |
...
plan (H1–H5) and the parallel \`$DEV-avbcontrol-*\` / \`$DEV-fullgt-*\`
...
\`\`\`bash
export REMOTE_HOST=openstatestack@192.168.1.4
export REMOTE_BUILD_DIR=$ROOT/releases/desktop-flash/$DEV-latest
export REMOTE_KEY_DIR=$ROOT/releases/desktop-flash/$DEV-latest
DEVICE=$DEV bash scripts/flash-from-remote.sh
...
stage_fullgt() {
  ...
  local pkmd=""
  for c in "$ROOT/releases/desktop-flash/$DEV-latest/avb_pkmd_testkey.bin" \
           "$ROOT/releases/desktop-flash/$DEV-memtagfix2-20260728-101648/avb_pkmd_testkey.bin"; do
    [ -f "$c" ] && pkmd="$c" && break
  done
  [ -n "$pkmd" ] && cp -f "$pkmd" "$DEST/avb_pkmd.bin"

  cat > "$DEST/README-FLASH-DESKTOP.md" <<EOF
# ${DEV} FULL-GT diagnostic build — $STAMP (NOT $DEV-latest)

**Purpose (RCA HOLD H3):** GT boot/init_boot/vendor_boot chain + GT vendor +
GT system\* + GT dlkm, ALL from ONE current \`out/target/product/$DEV\` build
...
**Do not point \`$DEV-latest\` at this bundle** — GT boot-chain ABL
...
# ===========================================================================
stage_avbcontrol() {
  local src_super="$ROOT/releases/desktop-flash/$DEV-stocksuper-avbok/super.img"
  need "$src_super"
  need "$STOCKCTL/bootloader.img"; need "$STOCKCTL/radio.img"

  local boot_src="$ROOT/releases/desktop-flash/$DEV-stocksuper-avbok"
  ...
  # Populate factory logicals from $DEV-stock-userspace / stock-control.
  local stock_userspace="$ROOT/releases/desktop-flash/$DEV-stock-userspace"
  ...
  cat > "$DEST/README-FLASH-DESKTOP.md" <<EOF
# ${DEV} STOCK + AVB Flags:3 control build — $STAMP (NOT $DEV-latest)
...
**Do not point \`$DEV-latest\` at this bundle.**
...
stage_gtsystemonstock() {
  ...
  [ -f "$STOCKCTL/init.insmod.$DEV.cfg" ] && cp -f "$STOCKCTL/init.insmod.$DEV.cfg" "$DEST/init.insmod.$DEV.cfg"
  cp -f "$vbmeta_flags3" "$DEST/vbmeta.img"
  cp -f "$vbmeta_flags3" "$DEST/vbmeta_system.img"
  cp -f "$vbmeta_flags3" "$DEST/vbmeta_vendor.img"
  cp -f "$vbmeta_flags3" "$DEST/vbmeta_valid_flags3.img"
  ...
  cat > "$DEST/README-FLASH-DESKTOP.md" <<EOF
# ${DEV} INVERTED bisect — GT system.img on stock everything — $STAMP
# MODE=$MODE
...
log "  out:     $OUT"
log "  donors:  stock=$STOCK stockctl=$STOCKCTL rescue=$RESCUE (id=$STOCK_ID)"
log "  dest:    $DEST"
