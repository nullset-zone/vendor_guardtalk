# Q-RANGO-BOOT-POSTINIT Evidence

**Task:** Q-RANGO-BOOT-POSTINIT  
**QA run:** 2026-08-02T15:09:07Z (build host; shell verify ~15:08–15:09 UTC)  
**Depends on:** T-RANGO-BOOT-POSTINIT ✅ APPROVED (static CONDITIONAL GO; on-device HOLD)  
**Preserved Backend report:** `.agent-comm/inbox/TO_ARCHITECT_T-RANGO-BOOT-POSTINIT.md`

## Host stamp verify (`rango-20260802-150440` / MODE=stockapexcfg)

| Check | Result |
|-------|--------|
| Stamp dir present | `releases/desktop-flash/rango-20260802-150440/` + `system.img` + `SHA256SUMS` + `README-FLASH-DESKTOP.md` |
| `debugfs dump` stamp `/system/bin/init` vs `132759` | **`init_vs_132759_MATCH`** size **2938144** (GT retained) |
| stamp init vs stock | **DIFFERS** (2938144 ≠ 2771368) — `init_vs_stock_DIFFERS_PASS` |
| stamp init vs `141438` (stockinit) | **DIFFERS** — `init_vs_141438_DIFFERS_PASS` (not stockinit) |
...
| stamp init.rc vs `130756` / `132759` / `141438` | **DIFFERS** (57115 ≠ 58090) — graft effective |
| stamp `/system/etc/init/apexd.rc` vs stock | **`apexd_rc_stock_MATCH`** |
| stamp init.rc has `perform_apex_config` + `exec_start apexd-bootstrap` | **PASS** |
| `debugfs … com.android.virt.apex` on `150440` | `File not found by ext2_lookup` |
| virt in `/system/apex` listing | none (`virt_ABSENT_PASS`) |
| SHA256SUMS `150440` | **22/22 OK** |
| Stamp README | `MODE=stockapexcfg`; GT init retained; stock init.rc; `REMOTE_BUILD_DIR=…/150440`; HOLD honesty; fail-class table |
...
## `--link-latest` refuse (stockapexcfg)

```text
MODE=stockapexcfg ./vendor/guardtalk/scripts/stage-rango-release.sh --link-latest
→ ERROR: MODE=stockapexcfg is a bisect stamp — refuse --link-latest (...)
exit:1
...
```text
BUNDLE=releases/desktop-flash/rango-20260802-150440 \
...
E2E_EXIT=0
bundle → rango-20260802-150440
```

Raw log: `/tmp/q-rango-boot-postinit-e2e.log`.
...
fastboot → not in PATH on build host (host binary present but unused)
~/rango-dmesg-apexcfg*.txt → absent on build host
```
...
export REMOTE_BUILD_DIR=/mnt/Big-Storage/GuardTalk/GrapheneOS-worktree/releases/desktop-flash/rango-20260802-150440
DEVICE=rango ./flash-from-remote.sh
fastboot oem dmesg | tee ~/rango-dmesg-apexcfg-150440.txt
adb devices
```

Exact stamp: **`rango-20260802-150440`**.  
Confirm step 0 `build dir` AND `key dir` both end with `rango-20260802-150440`.  
Do **not** unset `REMOTE_BUILD_DIR` (that would flash durable `130756` via `rango-latest`).  
Do **not** flash fullgt.  
Do **not** use `rango-latest` for this bisect.  
Slot A was drained after `141438` — **`fastboot set_active a` first**.

### Classification plan (if fail / no adb)

| Signal in `oem dmesg` / boot | Class | Meaning |
|-----------------------------|-------|---------|
| `Reboot mode: 0xfc` / `reboot bootloader` / `bootstrap-apexd-failed` ~18s | apexd-bootstrap residual | Stock **init.rc** graft (with GT init + stock apexd) **insufficient**; next residual probes (hwasan / SELinux / further early init) |
| `KP: … exitcode=0x00000100` / `0xbaba` / mode `0x0` ~61s | init exit status 1 | Same class as `141438` stockinit — init.rc graft did not change class |
...
- [x] Stamp `150440` GT init MATCH `132759` (2938144); NOT stock init; apexd stock MATCH (1083368); init.rc stock MATCH (57115)
- [x] virt absent; `rango-latest` still `130756`; `--link-latest` refused for stockapexcfg (exit 1)
- [x] Host E2E 49/0 on `BUNDLE=…/150440`; SHA256SUMS 22/22; no new `scripts/flash-*.sh`
- [x] On-device HOLD + Mac flash plan with `set_active a` + `REMOTE_BUILD_DIR=…/150440` + classify `0xfc` vs `0x00000100` vs `0x7f00` (no invent PASS)
